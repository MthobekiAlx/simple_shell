...
int set_work_directory(program_data *data, char *new_dir)
{
	...
	if (!str_comp(old_dir, new_dir, 0))
	{
		...
		env_setkey("PWD", new_dir, data);
	}
	env_setkey("OLDPWD", old_dir, data);
	return (0);
}

/**
 * built_in_help - shows the environment where the shell runs
 * @data: struct for the program's data
 * Return: zero if sucess, or other number if its declared in the arguments
 */
...
/**
 * built_in_alias - add, remove or show aliases
 * @data: struct for the program's data
 * Return: zero if sucess, or other number if its declared in the arguments
 */
